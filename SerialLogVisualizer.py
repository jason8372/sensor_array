...
filename = start_time.strftime("serial_data_log_%Y-%m-%d_%H-%M-%S.csv")
csv_file = open(filename, 'w', newline='')
csv_writer = csv.writer(csv_file)
csv_writer.writerow(['Timestamp'] + DATA_KEYS + ['GASID'])  # 헤더 작성
...
def update(frame):
    ...
    if line.startswith("D:"):
        try:
            ...
            csv_writer.writerow([timestamp] + values + [label])
            csv_file.flush()
